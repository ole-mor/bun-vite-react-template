...
const ItemPreview: React.FC = () => {
  const { poseCode } = useParams<{ poseCode: string }>();

  const { candyId } = useParams<{ candyId: string }>();
  ...
  const currentItem = candyItems.find(
    (item) => generateItemId(UTF8ToAscii(item.name)) === UTF8ToAscii(candyId)
  );
  ...
  if (!currentItem) {
    return (
      <div className="p-8 text-center flex flex-col items-center justify-center h-full">
        ...
        <p className="mb-4 text-gray-300">Kunne ikke finne smågodt med ID: {candyId}</p>
  ...
  return (
    <div className="h-[100svh] flex flex-col overflow-hidden">
      ...
      <Link className="absolute text-black h-0 text-xl p-4 z-10" to={`/pose/${poseCode}`}>
        &larr; Gå tilbake
      </Link>
      <AmountSelector
      ...
      <div className="flex h-full w-full flex-col md:flex-row flex-grow min-h-0">
        ...
        <div className='absolute right-0 p-2 bg-transparent hover:bg-none text-black z-20'>
        </div>
        <div
            ref={dragHandleRef}
